...
<p class="text-white">Laporan Anggaran Pendapatan Belanja Desa {{ $desa->nama_desa }}, masyarakat dapat dengan mudah
    mengetahui informasi mengenai Laporan Anggaran Pendapatan Belanja Desa {{ $desa->nama_desa }}.</p>
@endsection
            ...
            <table class="table table-hover table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <th class="text-right">Rp. {{ substr(number_format($anggaran_kelompok, 2, ',', '.'),0,-3) }}
                        </th>
                        <th class="text-right">Rp. {{ substr(number_format($realisasi_kelompok, 2, ',', '.'),0,-3) }}
                        </th>
                        <th class="text-right">Rp.
                            {{ substr(number_format($anggaran_kelompok - $realisasi_kelompok, 2, ',', '.'),0,-3) }}</th>
                        <th class="text-right">
                            @php
                            try {
                    ...
                    <tr>
                        ...
                        <td class="text-right">Rp. {{ substr(number_format($anggaran - $realisasi, 2, ',', '.'),0,-3) }}
                        </td>
                        <td class="text-right">
                            @php
                            try {
                    ...
                    <tr class="bg-primary text-white font-weight-bold ">
                        ...
                        <td class="text-right">Rp.
                            {{ substr(number_format($anggaran_jenis - $realisasi_jenis, 2, ',', '.'),0,-3) }}</td>
                        <td class="text-right">
                            @php
                            try {
                    ...
                    <tr class="bg-primary text-white font-weight-bold ">
                        ...
                        <td class="text-right">
                            @php
                            try {
                            $persen = (($data['pendapatan_realisasi'] -
                            $data['belanja_realisasi'])/($data['pendapatan_anggaran'] - $data['belanja_anggaran'])) *
                            100;
                            echo number_format((float)$persen, 2, '.', '') . "%";
                            } catch (\Throwable $th) {
                            echo "-";
                    ...
                    <tr class="bg-primary text-white font-weight-bold ">
                        ...
                        <td class="text-right">Rp.
                            {{ substr(number_format($data['pembiayaan_netto_anggaran'], 2, ',', '.'),0,-3) }}</td>
                        <td class="text-right">Rp.
                            {{ substr(number_format($data['pembiayaan_netto_realisasi'], 2, ',', '.'),0,-3) }}</td>
                        <td class="text-right">Rp.
